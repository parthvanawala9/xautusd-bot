...
# v80.0 - SIMPLE BRACKET SL ONLY
#
# STRATEGY:
#   DAY HIGH BREAK  -> LONG  -> BRACKET SL = DAY LOW
#   DAY LOW BREAK   -> SHORT -> BRACKET SL = DAY HIGH
#
# NO:
#   - 1:5 PROFIT TARGET
#   - HALF PROFIT BOOKING
#   - PARTIAL EXIT
#   - TARGET BASED SL MODIFICATION
#   - MOVE SL TO ENTRY
#
# SL IS PLACED AS A NATIVE DELTA BRACKET STOP LOSS.
# =====================================================================
...
# Trading starts at 05:45 IST.
# 05:30 - 05:45 is the range-forming period.
...
class DeltaClient:
    ...
    def position(self, product_id):
        ...
        return {
            ...
            "stop_loss": decimal_or_none(
                pos_item.get("stop_loss")
            ),

            "liquidation_price": decimal_or_none(
                pos_item.get("liquidation_price")
            ),

            "bankruptcy_price": decimal_or_none(
                pos_item.get("bankruptcy_price")
            ),

            "margin": decimal_or_none(
                pos_item.get("margin")
            ),

            "mark_price": decimal_or_none(
    ...
    def set_leverage(
        ...
        try:
            ...
            logging.info(
                f"Leverage successfully set to "
                f"{leverage_val}x for product "
                f"ID {product_id}"
            )

        except Exception as e:
            logging.error(
                f"Failed to set leverage to "
                f"{leverage_val}x for product "
                f"ID {product_id}: {e}"
            )
            raise
    ...
    # -----------------------------------------------------------------
    # NATIVE DELTA BRACKET STOP LOSS
    #
    # ONLY STOP LOSS.
    # NO TAKE PROFIT.
    # NO PARTIAL EXIT.
    ...
    # -----------------------------------------------------------------
    # REDUCE POSITION MARKET
    #
    # Kept only for emergency/manual functionality.
    # NOT USED FOR PROFIT BOOKING.
    # -----------------------------------------------------------------

    def reduce_position_market(
        self,
        product_id,
        size,
        direction
...
class AccountBot:
    ...
    def __init__(
        ...
        self.unique_id = (
            f"{account_id}_{self.symbol}"
        )

        self.account_name = (
            f"{account_name} [{self.symbol}]"
        )
        ...
        with self.lock:
            ...
            if size != 0:
                ...
                return {
                    ...
                    "message": (
                        f"Bot [{self.symbol}] "
                        f"started with existing position."
                    )
                }
    ...
    def estimate_liquidation_price(
        ...
        if self.product:
            m_raw = self.product.get(
                "maintenance_margin",
                0
            )

            t_raw = self.product.get(
    ...
    def enter(
        ...
        # -------------------------------------------------------------
        # CANCEL ANY OLD ORDERS BEFORE NEW ENTRY
        # -------------------------------------------------------------
        ...
        for lev in ladder:
            ...
            # LONG:
            # liquidation must remain BELOW Day Low.
            if (
                direction == "LONG"
                and candidate_liq
                >= Decimal(str(sl_level))
            ):
                continue

            # SHORT:
            # liquidation must remain ABOVE Day High.
            if (
                direction == "SHORT"
                and candidate_liq
                <= Decimal(str(sl_level))
            ...
            try:
                ...
                logging.warning(
                    f"[{self.symbol}] "
                    f"Leverage {lev}x rejected: {e}. "
                    f"Trying next step down."
                )
        ...
        if not order_done:
            ...
            logging.error(
                f"[{self.symbol}] "
                f"Liquidation-safe order entry failed: "
                f"{last_error}"
            )
        ...
        if (
            ...
            logging.error(
                ...
                f"Closing any accidental position."
            )
        ...
        # -------------------------------------------------------------
        # VERIFY ACTUAL LIQUIDATION PRICE
        # -------------------------------------------------------------
        ...
        actual_liq = margined_data.get(
            "liquidation_price"
        )

        if (
            actual_liq is None
            or str(actual_liq).strip()
            in ("", "None", "null")
        ):

            logging.error(
                f"[{self.symbol}] "
                f"POST-ENTRY LIQUIDATION IS NONE! "
                f"Closing position."
            )
        ...
        try:
            ...
            if not is_safe:
                ...
                logging.error(
                    f"[{self.symbol}] "
                    f"UNSAFE LIQUIDATION! "
                    f"Liq={actual_liq_dec} | "
                    f"SL={sl_dec}. "
                    f"Closing position."
                )
            ...
            logging.error(
                f"[{self.symbol}] "
                f"Error validating liquidation: {e}"
            )
        ...
        # -------------------------------------------------------------
        # NATIVE BRACKET STOP LOSS ONLY
        #
        # LONG  -> DAY LOW
        # SHORT -> DAY HIGH
        #
        # THERE IS NO TARGET.
        # THERE IS NO PARTIAL BOOKING.
        # -------------------------------------------------------------
        ...
        try:
            ...
            self.client.place_bracket_sl_order(
                self.product_id,
                direction,
                sl_dec
            )
            ...
            logging.error(
                f"[{self.symbol}] "
                f"FAILED TO PLACE BRACKET SL: {e}"
            )

        # -------------------------------------------------------------
        # IF BRACKET SL FAILED:
        ...
        self.active_trade = {
            ...
            "estimated_liquidation": (
                float(estimated_liq)
                if estimated_liq is not None
                else None
            ),
            "actual_liquidation": float(
                actual_liq_dec
            ),
    ...
    #
    # IMPORTANT:
    # There is NO profit target logic here.
    # There is NO partial profit logic here.
    # There is NO SL modification logic here.
    # -----------------------------------------------------------------
    ...
    def evaluate(self, price=None):
        ...
        with self.lock:
            ...
            # =========================================================
            # POSITION BECAME FLAT
            #
            # This means:
            # - Bracket SL may have triggered
            # - Manual/external close may have happened
            #
            # If we had an active trade, record it.
            # Then allow opposite breakout.
            # =========================================================
            ...
            if (
                ...
                # -----------------------------------------------------
                # CANCEL ANY REMAINING BRACKET/OLD ORDERS
                # -----------------------------------------------------
                ...
                if (
                    ...
                    # LONG SL -> SHORT only after
                    # price breaks Day Low downward.

                    if (
                        old_dir == "LONG"
                        and old_price >= self.day_low
                    ...
                    # SHORT SL -> LONG only after
                    # price breaks Day High upward.

                    elif (
                        old_dir == "SHORT"
                        and old_price <= self.day_high
            ...
            # =========================================================
            # FLAT:
            # CHECK NEW DAY HIGH / DAY LOW BREAKOUT
            # =========================================================
    ...
    def do_GET(self):
        ...
        if path == "/api/dashboard":
            ...
            for b in bots:
                ...
                if (
                    b.active_trade
                    and b.active_trade.get("sl")
                    is not None
                ):

                    active_sl = float(
                        b.active_trade.get("sl")
                    )
        ...
        if (data.success) {
            ...
            data.accounts.forEach(acc => {
                ...
                >
                    ...
                    <div
                        class="grid grid-cols-2 gap-2 bg-slate-900/80 p-3 rounded-xl border border-amber-500/30 text-xs"
                    >

                        <div>

                            <span
                                class="text-slate-400 text-[10px] block uppercase"
                            >
                                Locked Day High
                            </span>
                        ...
                        <div>
                            ...
                            <span
                                class="font-bold text-rose-400 text-sm"
                            >
                                ${
                                    acc.day_low !== null
                                    ? acc.day_low
                    ...
                    >
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option
                                        value="0.10"
                                        ${acc.balance_fraction==0.1?'selected':''}
                                    >
                                        10%
                                    </option>
                                    ...
                                    <option
                                        value="0.25"
                                        ${acc.balance_fraction==0.25?'selected':''}
                                    >
                                        25%
                                    </option>
                                    ...
                                    <option
                                        value="0.50"
                                        ${acc.balance_fraction==0.5?'selected':''}
                                    >
                                        50%
                                    </option>
                                    ...
                                    <option
                                        value="0.75"
                                        ${acc.balance_fraction==0.75?'selected':''}
                                    >
                                        75%
                                    </option>
                                    ...
                                    <option
                                        value="1.00"
                                        ${acc.balance_fraction==1.0?'selected':''}
                                    >
                                        100%
                                    </option>
                        ...
                        <button
                            ...
                            class="w-full bg-slate-700 hover:bg-slate-600 text-xs font-semibold py-1.5 rounded-lg transition"
                        >
                            Save Settings
                        </button>
                    ...
                    >
                        ...
                        >
                            ...
                            ${
                                ...
                                `
                                <div
                                    class="text-slate-500 text-center py-2"
                                >
...
if __name__ == "__main__":
    ...
    logging.warning(
        "DELTA PRO AUTOTRADER v80.0 STARTING..."
    )
    ...
    logging.warning(
        "NO 1:5 TARGET / NO PARTIAL PROFIT"
    )
